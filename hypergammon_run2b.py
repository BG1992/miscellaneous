...
with open('D://hypergammon//hyper_32a_52.csv', 'r', newline='') as csvfile:
    ...
    for row in _reader:
        tp = eval(row[0])
        if len(tp[0]) == 3 and len(tp[1]) == 2:
            if 0 not in tp[0] and 25 not in tp[1]:
                if min(tp[0]) > max(tp[1]):
...
ind = 0
with open('D://hypergammon//hyper_32_22.csv', 'r', newline='') as csvfile:
    _reader = csv.reader(csvfile, delimiter=';')
    for row in _reader:
        tp = eval(row[0])
        if tp not in states: states[tp] = Decimal(row[1])
        if ind % 10000 == 0: print(ind, row, len(states))
        ind += 1
...
def f():
    ...
    for state in states_list:
        ...
        if score not in [-1, 1] and not (len(state[0]) <= 2 and len(state[1]) <= 2):
            if not (0 not in tp[0] and 25 not in tp[1] and min(tp[0]) > max(tp[1])):
                em = 0
                for roll in rolls:
                    if state[2] == 1:
...
while 1:
    ...
    ind += 1
    with open('D://hypergammon//hyper_32b_'+str(ind)+'.csv', 'w', newline='') as csvfile:
