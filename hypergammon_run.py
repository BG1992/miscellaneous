        ...
        for r_black in range(3):
            for comb_black in combinations_with_replacement(posses_black.difference(_comb_white), r_black):
                ...
                if score is not None:
                    states[(comb_white, comb_black, 1)] = score
                    ind += 1
                    states[(comb_white, comb_black, -1)] = score
                    ind += 1
...
def f():
    ...
    for state in states:
        ...
        if score not in [-1, 1]:
            ...
            diffs += abs(em - score)
            states[state] = em
            if score != 0 and abs(em - score) > 0: updates += 1
        if i % 1000 == 0: print(i, len(states))
        i += 1
...
while 1:
    ...
    ind += 1
    with open('D://hypergammon'+str(ind)+'.csv', 'w', newline='') as csvfile:
        _writer = csv.writer(csvfile, delimiter= ';')
        ind2 = 0
        for el in states:
            item = [el, states[el]]
            _writer.writerow(item)
            if ind2 % 10000 == 0: print(ind2, len(states))
            ind2 += 1