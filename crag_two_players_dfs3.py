...
# main_base.execute('create table crag_two_players (p0 INTEGER, p1 INTEGER, q0 INTEGER, q1 INTEGER, '
...
#                   'PRIMARY KEY (p0, p1, q0, q1, d1, d2, d3, remained))')
#
# main_base.commit()
...
def dfs(curr_state, ftm, depth, md):

    if depth > md: return None
    visited[0] += 1
    if visited[0] % 100000 == 0:
        print(visited, depth, md, len(memo))
        print(curr_state)
    state = curr_state[0]
    ...
    if state[-1] == 0 and state[26] != 0:
        ...
        for cat in categories:
            if state[categories[cat] + 13 * (int(ftm) ^ 1)] == 'NULL':
                ...
                temp_value = dfs(_next_state, not ftm, depth + 1, md)
        ...
        for comb in combinations_with_replacement([i for i in range(1, 7)], 3):
            ...
            new_state_ev = dfs(_next_state, ftm, depth + 1, md)
            if new_state_ev is not None:
        ...
        for sd in saved_dices_set:
            ...
            temp_value = ev / ct
            if temp_value is not None: minimax = temp_value
            if ftm:
                if temp_value > minimax:
                    minimax = temp_value
    ...
    if minimax is not None: add_node(conversed_state, minimax, action)
    return minimax

visited = [0, 0]
curr_state = (('NULL',)*26 + (0,0,0,0), (0,0,244,244))
for md in range(5, 78, 3):
    dfs(curr_state, True, 0, md)
