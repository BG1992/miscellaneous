...
def dfs(states, curr_tp, sign, depth):
    ...
    minimax = -sign
    if depth <= 2:
        print(curr_tp, depth, checked_tp[0], len(memo))
    ...
    for m in curr_board:
        ...
        if temp_result is not None:
            if temp_result == sign:
                ...
                return sign
        temp_hval = base.execute('select explained, all_nodes from states_indicates where ' \
                    '(white_k, white_num, black_k, black_num, wtm) = ' + str(curr_tp + (sign,))).fetchone()
        if temp_hval is not None:
            next_states[format_tp(temp_state)] = temp_hval[0]/temp_hval[1]
        else:
            next_states[format_tp(temp_state)] = 0
    #states_memo.add(frozenset(states.keys()))
    states[format_tp(curr_tp)] = sign
    _next_states = list(next_states.items())
    _next_states.sort(key= lambda x: -x[1])
